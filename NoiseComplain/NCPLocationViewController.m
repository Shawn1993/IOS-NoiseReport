...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (self.isUsePoiResult) {
        BMKPoiInfo *poiInfo = self.poiResult.poiInfoList[row];
        cell.textLabel.text = poiInfo.name;
        cell.detailTextLabel.text = poiInfo.address;
    } else {
        cell.textLabel.text = self.suggestionResult.keyList[row];
    }
    return cell;
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    NSUInteger row = (NSUInteger) indexPath.row;
    if (self.isUsePoiResult) {
        self.pinCoordinate = [self.poiResult.poiInfoList[row] pt];
    } else {
        CLLocationCoordinate2D coordinate;
        [self.suggestionResult.ptList[row] getValue:&coordinate];
